...
describe("Tier 2 Shared — Tier 3 Escalation", () => {
  let projectRoot: string;
  let logDir: string;

  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tier2-escalation-"));
  ...
  it("consumer BLOCKED 2x → Tier 3 escalation", async () => {
  ...
  it("error log records tier3_escalation resolution", async () => {
    const runner: RunnerFn = async (card: DispatchCard) => {
      if (card.role === "planner") return { tasks_md: "# tasks" };
      if (card.role === "reviewer") return {
        review_report: "ok", disposition_recommendation: "PASS" as const, issues: [],
    ...
    expect(errors.some((e) => e.resolution === "tier3_escalation")).toBe(true);
  });

  it("undiscovered shared surface → Tier 3 escalation", async () => {
    const runner: RunnerFn = async (card: DispatchCard) => {
      if (card.role === "planner") return { tasks_md: "# tasks" };
      if (card.role === "reviewer") return {
        review_report: "ok", disposition_recommendation: "PASS" as const, issues: [],
      ...
      // Consumer blocked on undiscovered surface
      return {
        status: "blocked" as const, touched_files: [],
        changeset: "x", delta_stub: "x",
        evidence: { build_pass: false, test_pass: false, test_summary: "blocked" },
        blocked_on: { reason: "shared_pending" as const, surface: "src/types/new-shared.ts", owner_id: "specialist-1" },
      };
    };
  ...
  it("owner crash → retry exhausted → consumer skip → failure", async () => {
    const runner: RunnerFn = async (card: DispatchCard) => {
      if (card.role === "planner") return { tasks_md: "# tasks" };
      if (card.role === "reviewer") return {
        review_report: "ok", disposition_recommendation: "PASS" as const, issues: [],
      };
      if (card.is_shared_owner) throw new Error("owner always crashes");
      return {
    ...
    expect(result.success).toBe(false);
    // Consumer should not have run (owner failed)
    const consumerSettled = result.specialist_results.settled.filter(
      (s) => !s.id.includes("specialist-1"),
    );
    // Either empty or no consumer results
    expect(result.specialist_results.all_succeeded).toBe(false);
  });
});
